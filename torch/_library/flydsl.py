...
def wrap_flydsl(
    ...
    *,
    ...
    ``mutates_args`` names the tensor arguments written by the launcher. When
    ...
    if not runtime_available(minimum_version=(0, 2, 3)):
        raise RuntimeError(
            "wrap_flydsl requires the optional `flydsl` runtime version 0.2.3 "
            "or newer on a ROCm-enabled build"
        )
    ...
    mutations = None if mutates_args is None else frozenset(mutates_args)
    if mutations is not None:
        ...
        compile_time_parameters = {
            parameter_names[idx] for idx in compile_time_arg_indices
        }
