...
def _mutated_arg_indices(launcher_idx: int, args: tuple[Any, ...]) -> tuple[int, ...]:
    from torch._higher_order_ops.flydsl_kernel_wrap import flydsl_launcher_side_table

    registration = flydsl_launcher_side_table.get_registration(launcher_idx)
    if registration.mutates_args is None:
        return tuple(
            idx for idx, arg in enumerate(args) if isinstance(arg, ir.TensorBox)
        )

    parameter_names = tuple(registration.signature.parameters)
    return tuple(
        idx
        for idx, (name, arg) in enumerate(zip(parameter_names, args))
        if name in registration.mutates_args and isinstance(arg, ir.TensorBox)
    )
...
def lower_flydsl_kernel(
    *,
    ...
    UserDefinedFlyDSLKernel(
        ...
        kernel_args=args,
        mutated_arg_indices=_mutated_arg_indices(launcher_idx, args),
    )
