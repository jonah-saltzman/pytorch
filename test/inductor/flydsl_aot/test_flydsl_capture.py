...
HAS_FLYDSL = runtime_available(minimum_version=(0, 2, 3))
if HAS_FLYDSL:
    import flydsl.compiler as flyc
    import flydsl.expr as fx
    from flydsl.compiler import protocol
...
class FlyDSLCaptureTest(TestCase):
    ...
    def test_export_captures_explicit_launcher(self):
        ...
        self.assertEqual(
            [0],
            get_mutated_arg_indices(
                nodes[0].kwargs["launcher_idx"],
                example_args,
            ),
        )

    def test_unspecified_effects_conservatively_mutate_tensors(self):
        captured = torch.library.wrap_flydsl(_launcher)
        args = (torch.empty(2), torch.empty(3), 2)

        self.assertEqual([0, 1], get_mutated_arg_indices(captured.launcher_idx, args))
    ...
    def test_wrap_rejects_non_jit_function(self):
        with self.assertRaisesRegex(RuntimeError, "annotated with flydsl.compiler.jit"):
            torch.library.wrap_flydsl(lambda: None)
    ...
    def test_wrap_reports_missing_optional_runtime(self):
        with (
            ...
            torch.library.wrap_flydsl(object())

        available.assert_called_once_with(minimum_version=(0, 2, 3))
    ...
    def test_split_launcher_arguments_preserves_parameter_kinds(self):
        ...
        positional, keyword = split_flydsl_launcher_arguments(
            type("Launcher", (), {"func": staticmethod(launcher)})(),
            ("out", "inp", 8),
        )
    ...
    def test_split_launcher_arguments_rejects_variadic_parameters(self):
        ...
        with self.assertRaisesRegex(TypeError, "variadic parameters cannot be wrapped"):
            torch.library.wrap_flydsl(launcher)
    ...
    def test_flydsl_op_eager_path_bypasses_hop(self):
        eager_launcher = TraceableFlyDSLLauncher(
            _EagerLauncher(),
            frozenset({"out", "workspace"}),
        ...
        @torch.library.flydsl_op(
            "test_flydsl_capture::eager",
            mutates_args=(),
        )
        def flydsl_copy(inp: torch.Tensor) -> torch.Tensor:
            out = torch.empty_like(inp)
            workspace = torch.empty_like(inp)
            eager_launcher(out, workspace, inp)
            return out

        inp = torch.arange(8, dtype=torch.float32)
        with mock.patch(
            "torch._higher_order_ops.flydsl_kernel_wrap.flydsl_kernel_wrapper_mutation",
            side_effect=AssertionError("eager calls must bypass the HOP"),
        ):
            actual = flydsl_copy(inp)

        torch.testing.assert_close(actual, inp)

    def test_flydsl_op_eager_path_validates_runtime_arguments(self):
        eager_launcher = TraceableFlyDSLLauncher(
            _EagerLauncher(),
            frozenset({"out", "workspace"}),
            jit_argument_type=protocol.JitArgument,
        ...
        @torch.library.flydsl_op(
            "test_flydsl_capture::eager_runtime_argument_validation",
            mutates_args=(),
        )
        def flydsl_copy(inp: torch.Tensor) -> torch.Tensor:
            out = torch.empty_like(inp)
            workspace = torch.empty_like(inp)
            eager_launcher(out, workspace, fx.Int32(1))
            return out

        with self.assertRaisesRegex(TypeError, "graphable PyTorch values"):
            flydsl_copy(torch.ones(4))

    def test_flydsl_op_eager_path_validates_mutations(self):
        eager_launcher = TraceableFlyDSLLauncher(
            _EagerLauncher(),
            frozenset({"inp"}),
        )
        ...
        @torch.library.flydsl_op(
            "test_flydsl_capture::eager_mutation_validation",
            mutates_args=(),
        )
        def flydsl_copy(inp: torch.Tensor) -> torch.Tensor:
            out = torch.empty_like(inp)
            workspace = torch.empty_like(inp)
            eager_launcher(out, workspace, 1)
            return out

        with self.assertRaisesRegex(TypeError, "mutated arguments must be tensors"):
            flydsl_copy(torch.ones(4))

    @unittest.skipUnless(torch.cuda.is_available(), "requires a GPU")
    def test_flydsl_op_eager_path_rejects_non_default_stream(self):
        eager_launcher = TraceableFlyDSLLauncher(
            _EagerLauncher(),
            frozenset({"out", "workspace"}),
        )
        ...
        @torch.library.flydsl_op(
            "test_flydsl_capture::eager_non_default_stream",
            mutates_args=(),
        )
        def flydsl_copy(inp: torch.Tensor) -> torch.Tensor:
            out = torch.empty_like(inp)
        ...
        with (
            ...
            flydsl_copy(torch.ones(4, device="cuda"))
